...
export default function AreaAssignedPersonnelModal({
		...
		try {
			const payload = {
				...
				validFrom: validFromInput
					? formatToOffsetDateTime(validFromInput)
					: null,
				validTo:
					isIndefinite || !validToInput
						? null
			...
			toast.success(
				`Đã gán thành công người dùng ${selectedUser.fullName} vào ${area.name}`,
			);
			setShowAddForm(false);
			setSelectedUser(null);
			setValidFromInput("");
	...
	const renderStatusBadge = (status) => {
		switch (status) {
			...
			case "UPCOMING":
				return (
					<span className="ap-badge ap-badge--upcoming">Sắp hiệu lực</span>
				);
			case "EXPIRED":
				...
				<div className="ap-modal-content">
					...
					{isFacilityManager && showAddForm && (
						<form
							...
							className="ap-add-form"
						>
							<div className="ap-add-form__header">
								<UserPlus
									size={16}
									className="ap-add-form__icon"
								/>
								<span className="ap-add-form__title">
									Gán người dùng vào khu vực
								</span>
							</div>
							...
							<div className="ap-add-form__grid">
								...
								<div className="ap-form-group">
									<label className="ap-form-label">
										Hiệu lực từ{" "}
										<span className="ap-form-hint">(Bỏ trống = bây giờ)</span>
									</label>
									<input
										type="datetime-local"
								...
								{/* Valid To */}
								<div className="ap-form-group">
									<label className="ap-form-label">Hiệu lực đến</label>
									<input
										type="datetime-local"
										className="ap-form-input"
										value={validToInput}
								...
								<tbody>
									{personnelList.map((item) => {
										...
										return (
											<tr
												key={item.id}
												className={isRevoked ? "ap-row--revoked" : ""}
											>
												<td className="ap-cell--code">
													{item.user?.userCode || "—"}
												</td>
												...
												<td>
													<span
														className={`ap-role-pill role--${item.user?.role}`}
													>
				...
				>
					<div className="ap-edit-form">
						<div className="ap-form-group">
							...
							/>
							<label
								className="ap-form-checkbox-inline"
								style={{ marginTop: "8px" }}
							>
								<input
									type="checkbox"
				...
				>
					<div className="ap-revoke-form">
						<div className="ap-revoke-warning">
							<AlertCircle
								size={18}
								className="ap-revoke-warning__icon"
							/>
							<div className="ap-revoke-warning__text">
								Hành động này sẽ <strong>thu hồi quyền cố định</strong> của
								người dùng tại khu vực này. Người dùng sẽ không thể vào khu vực
								trừ khi được cấp quyền lại hoặc có đơn được duyệt.
							</div>
						</div>
