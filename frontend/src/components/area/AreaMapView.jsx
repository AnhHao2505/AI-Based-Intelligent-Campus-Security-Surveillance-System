					...
					<div className="zone-draw-bar">
						<div className="zone-draw-bar__info">
							<span>
								Đang vẽ:{" "}
								<strong>
									{areas.find((a) => a.id === drawingAreaId)?.name || ""}
								</strong>
							</span>
							<span className="zone-draw-bar__sep">·</span>
							<span className="zone-draw-bar__count">
								Đã đặt {draftVertices.length} đỉnh
							</span>
							<span className="zone-draw-bar__hint">
								(Nhấp lên ảnh để thêm đỉnh)
							</span>
						</div>
					...
					{!selectedPlan || imageError ? (
						<div className="zone-canvas-empty">
							...
							<div className="zone-canvas-empty__desc">
								Chưa có sơ đồ mặt bằng cho Tòa {selectedBuilding || "—"} · Tầng{" "}
								{selectedFloor || "—"}.
							</div>
							<button
								type="button"
								...
								{mapPolygons.map((area) => {
									...
									return (
										<polygon
											key={area.id}
											points={points}
											className={`zone-map-polygon ${(getLevelPolygonClass || getLevelPolygonClassHelper)(area.areaLevel || area.level)} ${
					...
					<div className="zone-rail-list">
						{sortedAreasForRail.length === 0 ? (
							...
							sortedAreasForRail.map((area) => {
								const isSelected = selectedAreaId === area.id;
								const inScope =
									area.building === selectedBuilding &&
									area.floor === selectedFloor;
								const hasGeo =
									area.hasGeometry ||
									(area.geometry &&
										...
										<div className="zone-rail-item__right">
											{inScope ? (
												hasGeo ? (
													<Check
														size={16}
														className="zone-status-icon zone-status-icon--check"
													/>
												) : (
													<EyeOff
														size={16}
														className="zone-status-icon zone-status-icon--none"
													/>
												)
											) : (
				...
				<div className="zone-rail-card zone-rail-card--detail">
					{!selectedArea ? (
						<div className="zone-detail-empty">
							<p>
								Chọn một khu vực trên bản đồ hoặc trong danh sách để xem cấu
								hình chi tiết.
							</p>
						</div>
					) : (
						<div className="zone-detail-content">
							...
							<div className="zone-detail-meta">
								...
								<div className="zone-detail-meta-row">
									...
									<span className="zone-detail-meta-val">
										Tòa {selectedArea.building || "—"}, Tầng{" "}
										{selectedArea.floor || "—"}
									</span>
								</div>
								...
								{isAdmin &&
									...
									(confirmDeleteId === selectedArea.id ? (
										<div className="zone-inline-confirm">
											<span className="zone-inline-confirm__prompt">
												Xoá hình?
											</span>
											<button
												type="button"
											...
											>
												{deletingGeometryId === selectedArea.id
													? "Đang xoá..."
													: "Xoá"}
											</button>
											<button
