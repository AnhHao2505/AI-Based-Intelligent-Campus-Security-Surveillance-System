...
import { faceDataApi } from '../api/faceDataApi';
...
export default function FaceManagementPage() {
  ...
  const loadFaces = async (searchKw = keyword, currentPage = page) => {
    setLoading(true);
    try {
      const data = await faceDataApi.getFaceList(searchKw, currentPage, 10);
  ...
  const handleSingleSubmit = async (e) => {
    ...
    try {
      await faceDataApi.registerSingleFace(
        singleForm.code,
        singleForm.fullName,
        frontFile,
        leftFile,
      ...
      showToast(`Đã nạp thành công hồ sơ [${singleForm.code}] vào Database & AI!`);
  ...
  const handleBulkSubmit = async (e) => {
    ...
    setBulkResult(null);
    try {
      const result = await faceDataApi.bulkImportZip(zipFile);
      setBulkResult(result);
      showToast(`Nạp thành công ${result.successCount}/${result.totalProcessed} người dùng!`);
      loadFaces(keyword, 0);
    } catch (err) {
  ...
  const handleDelete = async (id, code, name) => {
    if (!window.confirm(`Bạn có chắc muốn xóa hồ sơ của "${name}" (${code})?`)) return;

    try {
      await faceDataApi.deleteFace(id);
      ...
      <main className="face-main-content">
        ...
        <header className="face-header-section">
          <div>
            ...
            <h1 className="face-title-gradient">Quản Lý Dữ Liệu Khuôn Mặt (Face Dataset)</h1>
            <p className="face-subtitle">
              Nạp và quản lý tập ảnh chân dung mẫu 3 góc để AI đối soát nhận diện ra vào vùng cấm
            </p>
          </div>
          <div className="face-action-buttons">
            <button className="btn-glow-secondary" onClick={() => { setShowBulkModal(true); setBulkResult(null); }}>
              📦 Nạp Hàng Loạt (.ZIP)
            </button>
            <button className="btn-glow-primary" onClick={() => setShowSingleModal(true)}>
              ➕ Thêm Hồ Sơ Mới
            </button>
          </div>
        </header>
        ...
        <div className="filter-glass-bar">
          <form onSubmit={handleSearch} className="search-form-dark">
            ...
            <button type="submit" className="btn-search-submit">Tìm kiếm</button>
            {keyword && (
              <button
                type="button"
                className="btn-search-reset"
                onClick={() => { setKeyword(''); loadFaces('', 0); }}
              >
                Làm mới
              </button>
            )}
        ...
        <div className="table-glass-card">
          {loading ? (
            ...
            <div className="table-status-box">
              ...
              <p>Hãy bấm <strong>"Thêm Hồ Sơ Mới"</strong> hoặc <strong>"Nạp Hàng Loạt (.ZIP)"</strong> để nạp dữ liệu cho AI đối soát.</p>
            </div>
          ) : (
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <div className="face-thumbnails-group">
                          <div className="thumb-preview-box" title="Chính diện">
                            <img src={resolveImageUrl(item.imageFrontUrl)} alt="Front" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&auto=format&fit=crop&q=60'; }} />
                            <span>Chính diện</span>
                          </div>
                          <div className="thumb-preview-box" title="Góc trái">
                            <img src={resolveImageUrl(item.imageLeftUrl)} alt="Left" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&auto=format&fit=crop&q=60'; }} />
                            <span>Trái 45°</span>
                          </div>
                          <div className="thumb-preview-box" title="Góc phải">
                            <img src={resolveImageUrl(item.imageRightUrl)} alt="Right" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&auto=format&fit=crop&q=60'; }} />
                            <span>Phải 45°</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleDelete(item.id, item.code, item.fullName)}
                          title="Xóa hồ sơ này"
                        >
                          🗑️ Xóa
                        </button>
        ...
        <div className="modal-backdrop-blur">
          <div className="modal-glass-container">
            <div className="modal-header-glass">
              ...
              <button className="modal-close-icon" onClick={() => setShowSingleModal(false)}>✕</button>
            </div>
            <form onSubmit={handleSingleSubmit} className="modal-body-form">
                  ...
                  {frontPreview ? (
                    <div className="preview-box-active">
                      ...
                      <label className="btn-reselect">
                        Đổi ảnh
                        <input type="file" accept="image/*,.heic,.heif,.HEIC,.HEIF" onChange={(e) => handleImageChange(e, 'front')} hidden />
                      </label>
                    </div>
                  ) : (
                    <label className="dropzone-dark">
                      <span>Chọn ảnh</span>
                      <input type="file" accept="image/*,.heic,.heif,.HEIC,.HEIF" onChange={(e) => handleImageChange(e, 'front')} hidden required />
                    </label>
                  )}
                </div>
                ...
                <div className="angle-upload-card">
                  ...
                  {leftPreview ? (
                    <div className="preview-box-active">
                      ...
                      <label className="btn-reselect">
                        Đổi ảnh
                        <input type="file" accept="image/*,.heic,.heif,.HEIC,.HEIF" onChange={(e) => handleImageChange(e, 'left')} hidden />
                      </label>
                    </div>
                  ) : (
                    <label className="dropzone-dark">
                      <span>Chọn ảnh</span>
                      <input type="file" accept="image/*,.heic,.heif,.HEIC,.HEIF" onChange={(e) => handleImageChange(e, 'left')} hidden required />
                    </label>
                  )}
                </div>
                ...
                <div className="angle-upload-card">
                  ...
                  {rightPreview ? (
                    <div className="preview-box-active">
                      ...
                      <label className="btn-reselect">
                        Đổi ảnh
                        <input type="file" accept="image/*,.heic,.heif,.HEIC,.HEIF" onChange={(e) => handleImageChange(e, 'right')} hidden />
                      </label>
                    </div>
                  ) : (
                    <label className="dropzone-dark">
                      <span>Chọn ảnh</span>
                      <input type="file" accept="image/*,.heic,.heif,.HEIC,.HEIF" onChange={(e) => handleImageChange(e, 'right')} hidden required />
                    </label>
                  )}
                </div>
              </div>
              ...
              <div className="modal-footer-glass">
                <button type="button" className="btn-cancel-dark" onClick={() => setShowSingleModal(false)}>Hủy</button>
                <button type="submit" className="btn-glow-primary" disabled={singleSubmitting}>
                  {singleSubmitting ? 'Đang trích xuất Vector & Lưu...' : '💾 Lưu Hồ Sơ Vào Database'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="modal-backdrop-blur">
          <div className="modal-glass-container modal-lg">
            <div className="modal-header-glass">
              ...
              <button className="modal-close-icon" onClick={() => setShowBulkModal(false)}>✕</button>
            </div>
            <form onSubmit={handleBulkSubmit} className="modal-body-form">
              <div className="info-guide-dark">
                <h4>📌 Quy tắc đặt tên file ảnh trong file .ZIP:</h4>
                <p>Tên file theo chuẩn: <code>[MÃ_SỐ]_[HỌ_TÊN]_[GÓC_ẢNH].jpg</code></p>
                <ul>
                  <li><code>SE150001_Nguyen-Van-A_front.jpg</code> (Chính diện)</li>
                  <li><code>SE150001_Nguyen-Van-A_left.jpg</code> (Góc trái)</li>
                  <li><code>SE150001_Nguyen-Van-A_right.jpg</code> (Góc phải)</li>
                </ul>
              </div>
              ...
              {bulkResult && (
                <div className="bulk-report-dark">
                  ...
                  <p>Tổng số hồ sơ trong file: <strong>{bulkResult.totalProcessed}</strong></p>
                  <p className="text-success-neon">Thành công: <strong>{bulkResult.successCount}</strong> người</p>
                  {bulkResult.failedCount > 0 && (
                    <p className="text-danger-neon">Thất bại: <strong>{bulkResult.failedCount}</strong> người</p>
                  )}
                  {bulkResult.importedCodes?.length > 0 && (
                    <div className="tag-flex-wrap">
                      {bulkResult.importedCodes.map((code, i) => (
                        <span key={i} className="neon-code-tag">✓ {code}</span>
                      ))}
                    </div>
                  )}
                  {bulkResult.errorMessages?.length > 0 && (
                    <div className="error-log-box">
                      {bulkResult.errorMessages.map((msg, i) => (
                        <div key={i} className="error-line">⚠️ {msg}</div>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div className="modal-footer-glass">
                <button type="button" className="btn-cancel-dark" onClick={() => setShowBulkModal(false)}>Đóng</button>
                <button type="submit" className="btn-glow-primary" disabled={bulkSubmitting || !zipFile}>
                  {bulkSubmitting ? 'Đang giải nén & trích xuất AI...' : '🚀 Bắt Đầu Nạp Dataset'}
                </button>
              </div>
            </form>
          </div>
